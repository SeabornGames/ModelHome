...
class ArgparseEdgeType:
    ...
    def inx(self, name, viewname, arg):
        return (
        '      <param name="%s" type="enum" gui-text="%s" gui-description=%s>\n' %
...
class Boxes:
    ...
    def __init__(self):
        ...
        self.argparser._action_groups[
            1].title = self.__class__.__name__ + " Settings"
        ...
        for arg, default in kw.items():
            if arg == "x":
                ...
                self.argparser.add_argument(
                    ...
                    type=ArgparseEdgeType("efFcESikvfLt"),
                    choices=list("efFcESikvfLt"),
    ...
    def _buildObjects(self):
        ...
        edges.ClickSettings(self.thickness, True,
                            **self.edgesettings.get("Click", {})).edgeObjects(
            self)
        # Hinges
        edges.HingeSettings(self.thickness, True,
                            **self.edgesettings.get("Hinge", {})).edgeObjects(
            self)
        edges.ChestHingeSettings(self.thickness, True,
                                 **self.edgesettings.get("ChestHinge",
                                                         {})).edgeObjects(self)
        edges.CabinetHingeSettings(self.thickness, True,
                                   **self.edgesettings.get("CabinetHinge",
                                                           {})).edgeObjects(
            self)
        # Sliding Lid
    ...
    @restore
    def regularPolygonAt(self, x, y, corners, angle=0, r=None, h=None,
                         side=None):
    ...
    def regularPolygonWall(self, corners=3, r=None, h=None, side=None,
        ...
        if hole:
            self.hole(side / 2., h + edges[0].startwidth() + self.burn,
                      hole / 2.)
    ...
    def _latchGrip(self, length):
        ...
        self.grip(length / 2.0 - self.thickness / 2.0 - 0.2 * self.thickness,
                  self.thickness / 2.0)
        self.corner(90, self.thickness / 4.0)
    ...
    @restore
    def text(self, text, x=0, y=0, angle=0, align="", fontsize=10,
             color=[0.0, 0.0, 0.0]):
        """
        Draw text
            ...
            if self._splitWall(wallpieces, nr):
                for i in range(2):
                    ...
                    self.edges[edge](l / 2.0,
                                     bedBolts=self.getEntry(bedBolts,
                                                            wallcount),
                                     bedBoltSettings=self.getEntry(
                                         bedBoltSettings, wallcount))
                    wallcount += 1
            else:
                self.cc(callback, wallcount)
                self.edges[edge](l,
                                 bedBolts=self.getEntry(bedBolts, wallcount),
                                 bedBoltSettings=self.getEntry(bedBoltSettings,
                                                               wallcount))
                wallcount += 1
            self.corner(90, r)
    ...
    def surroundingWall(self, x, y, r, h,
        ...
        overallwidth = 2 * x + 2 * y - 8 * r + 4 * c4 + (self.edges[
                                                             "d"].spacing() +
                                                         self.edges[
                                                             "D"].spacing() + self.spacing) * pieces
    ...
    def flangedWall(self, x, y, edges="FFFF", flanges=None, r=0.0,
        ...
        for i in range(4):
            ...
            if flanges[i]:
                if edges[i] == "F":
                    self.fingerHolesAt(flanges[i - 1] + t - rl,
                                       0.5 * t + flanges[i], l,
                                       angle=0)
    ...
    def rectangularTriangle(self, x, y, edges="eee", r=0.0, num=1,
        ...
        if num > 1:
            ...
            height += 0.7 * r + edges[0].spacing() + edges[
                2].spacing() + self.spacing
        ...
        for n in range(num):
            ...
            if n % 2:
                self.moveTo(-edges[1].spacing() - 2 * self.spacing - edges[
                    -1].spacing(), height - edges[0].spacing(), 180)
            else:
                self.moveTo(
                    width + 1 * edges[1].spacing() - self.spacing - 2 * edges[
                        -1].spacing(), height - edges[0].spacing(), 180)
    ...
    def polygonWall(self, borders, edge="f", turtle=False,
        ...
        for i in range(0, len(borders), 2):
            ...
            if isinstance(next_angle, (int, float)) and next_angle < 0:
                length_correction = t * math.tan(
                    math.radians((-next_angle / 2)))
            else:
    ...
    def polygonWalls(self, borders, h, bottom="F", top="F", symetrical=True):
        ...
        while i < len(borders):
            ...
            with self.saved_context():
                ...
                top_edges.reverse()
                top_lengths.reverse()
                edges.CompoundEdge(self, top_edges, top_lengths)(
                    sum(top_lengths))
                self.edgeCorner(top, left, 90)
                left(h)
                self.edgeCorner(left, bottom, 90)
